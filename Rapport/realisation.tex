	...
	\begin{center}
		\fbox{
   		 \begin{minipage}[c]{1\textwidth}
  			\includegraphics[width=0.90\textwidth]{../Images/Rapport/results.png}  
		 \end{minipage}}
...
\begin{figure}[!ht]
    \begin{center}
        \fbox{
            \begin{minipage}[c]{1\textwidth}
              \includegraphics[width=0.90\textwidth]{../Images/Rapport/results_compare.png} 
         \end{minipage}}
        \caption{compare}
          \label{comparaison}
      \end{center}   
...
Sur cette même page, l'utilisateur à aussi la possibilité d'afficher les résultats de la commande qu'il vient de lancer. Pour cela, le fichier  \emph{parse\_results.php} récupère le fichier \emph{log.txt} dans lequel à été redirectionné les résultats générés par RegEfmtool.
...
Cette page parcourt le fichier log et selectionne les mots-clés à l'aide de la fonction \emph{item()} qui récupère toute les phrases suivient de deux points (\emph{:}).
...
Les mots-clés sont stocké dans un menu déroulant qui est obtenu par affichage de chaque valeur du tableaudans la balises HTLML (\emph{echo '<OPTION VALUE="' . $item[$numero] . '"> ' . $item[$numero] . ' </OPTION>';}). Ainsi l'utilisateur n'a plus qu'à selectionner la catégorie qu'il souhaite visualiser à partir du mot-clé.

Ainsi l'uilisateur peut faire le choix de visualiser l'ensemble du fichier \emph{log.txt} et ainsi la page le renvoi sur le fichier \emph{all\_results.php}, ou de ne visualiser qu'une catégorie, et il est rediriger sur le fichier \emph{display\_results.php}.

Dans le premier cas, l'utilisateur va obtenir une page semblable à la figure\ref{affichageResultats}. Cette page offre la possibilité à l'utilisateur de revenir en arrière à l'aide de deux boutons, dont un présent au dessus de l'affichage des résultats et l'autre, en dessous.\\
\begin{figure}[!ht]
	\begin{center}
		\fbox{
   		 \begin{minipage}[c]{0.8\textwidth}
...
Les résultats extraits sont affichés de façon claire et lisible pour l'utilisateur(Figure\ref{affichageResultats})grâce au fichier \emph{display\_results.php}. Ce dernier récupère le mot-clé selectionné dans le menu déroulant, de la page précédente, et le recherche dans l'ensemble du fichier.
\begin{lstlisting}
function display_results(){
	...
	//parcourt le fichier et stocke chaque ligne dans le tableau resultats et chaque mot cle dans le tableau item
	while (!feof($fichier)){
		$numligne++;
		$lignes = fgets($fichier, 1024);
		$posi = strpos($lignes, '|'); 
	...
	//Determine la position du mots cle $start dans le tableau resultats[]	
	if ($vrai){
	...
	//Cas ou $start contient un 'R' Majuscule
	...
	//l'autre cas : Determine la position du mot-cle delemitant la categorie a visualiser
	if ($vraie){
	...
	//Affichage des resultats sur la page HTML
...
\end{lstlisting}
Si ce dernier comporte un 'R' Majuscule, il n'est retourné que la ligne comportant le mot clé. Dans le cas contraire, un des mot-clé suivant délimitera la catégorie selectionnée. Cette recherche offre 2 possibilitées.
Dans le premier cas si le mot-clé est suivie de mots-clés contenant eu même un 'R' Majuscule, le mot-clé délimitant sera le premier qui ne possèdera pas de 'R' Majuscule.
Dans le second cas, le mot-clé suivant, dans la liste, servira de délimitant.
Cette démache est réalisé par la fonction \emph{display\_results()} du fichier \emph{display\_results.php}.\\

De plus il ya aussi la présence d'un bouton qui permet de retourner sur la page d'extraction des résultats.

\begin{figure}[!ht]
	\begin{center}
		\fbox{
   		 \begin{minipage}[c]{0.9\textwidth}
  			\includegraphics[width=0.20\textwidth]{../Images/Rapport/affichageExtraction.png}  
		 \end{minipage}}
		\caption{Page de recherche sur le log}
