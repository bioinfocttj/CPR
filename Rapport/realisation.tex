...
Ensuite, l'utilisateur est dirigé vers la page \emph{load\_files.php} (voir figure \ref{choix}) , qui permet à l'utilisateur d'aller "chercher" les fichiers aux formats \textit{sfile}, \textit{mfile}, \textit{rvfile}, \textit{grfile} et \textit{rfile}. Nous copions et renommons ces fichiers dans le dossier courant afin de pouvoir les utiliser dans \textit{regEfmtool}. Ce processus est faisable grâce à un  script PHP, dont voici un exemple pour le fichier au format \textit{sfile} :\\
...
Le fichier \emph{results.php} permet l'affichage des résultats de la commande générée précédemment. Celle-ci est exécutée grâce à la fonction \texttt{shell\_exec()}. Les fichiers de résultat et de log \footnote{Un fichier log est un fichier texte classique, reprenant de façon chronologique l'ensemble des événements qui ont affecté un système informatique et l'ensemble des actions qui ont résulté de ces événements.} sont analysés pour permettre leur affichage (Figure \ref{DiagResults}).\\
...
Suivant le navigateur utilisé, la récupération des fichiers par la fonction \texttt{move\_uploaded\_file()} ne fournit pas le m\^eme résultat, en particulier pour le dossier récupérant les fichiers. \\
...
L'affichage des résultats n'a été fait que superficiellement, d'où la présence de \textit{mode élémentaire 0} devant les noms de réactions, ou encore un simple affichage en tableau des résultats brut, indiquant la présence des réactions avec une valeur à 0 \textbf{\textcolor{red}{une valeur de quoi ?}}. L'amélioration de cette partie ne devant pas être trop gourmande en temps mériterait d'\^etre modifiée. \\
